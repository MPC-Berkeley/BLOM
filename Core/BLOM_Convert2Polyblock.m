...
%     end
    ...
    switch blockType
        case {'Sum','Add'} % add/subtract
            ...
            % FIX: add support for specifying a scalar >= 1 in List of signs
            % FIX: add support for Sum of Elements (List of signs = + or 1),
            % over all dimensions or specified dimension
            % Generally this section can be made much shorter, simpler, and
            % vectorized by removing the if statement. Go through the inputs,
            % concatenate a column of 1's or -1's for a scalar input, or a
            % speye() or -speye() for a vector input.
        ...
        case 'Abs'
            error('Absolute value currently not supported by BLOM');
        ...
        case 'Product' 
            ...
            switch mult_type
                case 'Element-wise(.*)'
                    ...
                    % FIX: This section can be made shorter, simpler, and
                    % vectorized by removing the if. Go through the inputs,
                    % concatenate a column of 1's or -1's for a scalar
                    % input, or a speye() or -speye() for a vector input.
                    if isempty(inportPlaces.matrix)
                        % all scalars
                ...
                case 'Matrix(*)'
                    P = [];
                    K = [];
                    error('Matrix multiplication currently not supported by BLOM');
            end
        ...
        case 'Trigonometry'
            ...
            switch mathFunction
                case 'tan'
                    ...
                    P = [specialSin*speye(totalInputs) spalloc(totalInputs,totalInputs,0); ...
                        specialCos*speye(totalInputs) speye(totalInputs)];
                    K = horzcat(speye(totalInputs),-speye(totalInputs));
...
%                     K = horzcat(speye(totalInputs),-speye(totalInputs));
                ...
                case 'atan'
                    ...
                    P = [speye(totalInputs), specialCos*eye(totalInputs); ...
                        sparse(totalInputs,totalInputs), specialSin*speye(totalInputs)];
                    K = horzcat(speye(totalInputs),-speye(totalInputs));
...
%                     K = horzcat(speye(totalInputs),-speye(totalInputs));
                case 'sincos'
                    ...
                    K = horzcat(speye(2*totalInputs),-speye(2*totalInputs));
                case 'sinh'
        ...
        case {'Math', 'Sqrt'}
            ...
            switch mathFunction
                ...
                case '10^u'
                    ...
                    K = horzcat(log(10)*speye(totalInputs),-speye(totalInputs));
                case 'hypot'
                    ...
                    if any(hypotInports ~= 1)
                        ...
                        if sum(hypotInports) == largeDim+3
                            ...
                            if largeDimInd <= 2
                                K = horzcat(speye(largeDim),ones(largeDim,1),-speye(largeDim));
                            else
                                K = horzcat(ones(largeDim,1),speye(largeDim),-speye(largeDim));
                            end
                        else
                            % both inputs are vectors
                            K = horzcat(speye(largeDim),speye(largeDim),-speye(largeDim));
                        end
                    else
