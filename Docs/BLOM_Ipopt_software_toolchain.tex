...
\item BLOM\_SetProblemData (section \ref{sec:BLOM_SetProblemData})
\item BLOM\_RunSolver (section \ref{sec:BLOM_RunSolver})
\end{itemize}
...
\begin{itemize}
\item A Matlab structure (\texttt{SolverStruct}) indicating the solver being used, the model name, and the indices of the external and state variables
\item The following files, saved to the current directory from Matlab (we may modify these names/formats in the future)
\begin{itemize}
...
\begin{itemize}
\item Solver data (\texttt{SolverStruct} structure) from BLOM\_ExportToSolver (section \ref{sec:BLOM_ExportToSolver}) indicating the solver being used, the model name, and the indices of the external and state variables
\item Optimization problem representation (\texttt{ModelSpec} structure) from BLOM\_ExtractModel (section \ref{sec:BLOM_ExtractModel})
...
\begin{itemize}
\item \texttt{testX0.dat} containing the initial starting guess for the optimization algorithm
\item \texttt{testFixed.dat} containing the values of the predicted external trajectories and initial states
\end{itemize}

\section{BLOM\_RunSolver} \label{sec:BLOM_RunSolver}

\subsection{What it does}

This is a Matlab wrapper to run the BLOM\_NLP executable (section \ref{sec:BLOM_NLP}) and return the optimization solution.
...
\item Optimization problem representation (\texttt{ModelSpec} structure) from BLOM\_ExtractModel (section \ref{sec:BLOM_ExtractModel})
\item BLOM options (TBD)
\item \texttt{result.dat} file from BLOM\_NLP executable (section \ref{sec:BLOM_NLP}), containing the optimization solution vector
\end{itemize}

\subsection{Output data}

\begin{itemize}
\item \texttt{SolverResult} structure in signal-named structure format
\item \texttt{ResultsVec} containing the raw optimization vector from \texttt{result.dat} 
\end{itemize}
...
\item BLOM\_SetProblemData (section \ref{sec:BLOM_SetProblemData})
\item BLOM\_RunSolver (section \ref{sec:BLOM_RunSolver})
\item BLOM\_GetStructOfInputVars extracts just the control input variables from the \texttt{ResultsVec} vector
\end{itemize}
...
\item BLOM\_SetProblemData (section \ref{sec:BLOM_SetProblemData})
\item BLOM\_RunSolver (section \ref{sec:BLOM_RunSolver})
\item BLOM\_GetStructOfInputVars, from the BLOM Matlab Library (section \ref{sec:BLOM_Matlab_Library})
\end{itemize}

\subsection{Input data}

\begin{itemize}
\item A matlab structure \texttt{model\_data} containing fields:
\begin{itemize}
\item \texttt{ModelSpec}, the optimization problem representation from BLOM\_ExtractModel (section \ref{sec:BLOM_ExtractModel})
\item \texttt{SolverStruct}, the solver data from BLOM\_ExportToSolver (section \ref{sec:BLOM_ExportToSolver})
\end{itemize}
\item Initial state values in signal-named structure format
\item Predicted external input trajectories in signal-named structure format
\item Warm start data from solution of previous MPC iteration (or a starting guess for first MPC iteration)
...
\begin{itemize}
\item A signal-named structure containing the optimal control variables
\item 
\end{itemize}
